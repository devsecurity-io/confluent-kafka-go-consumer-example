...
PROJECT_NAME:=confluent-kafka-go-consumer-example
...
all:
	...
	# amd64 build
	docker exec -t -w /go/src/$(PROJECT_NAME) build-container /bin/bash -c "GOOS=linux GOARCH=amd64 go build -v -ldflags \"-v -s -w -extldflags '-static' -X 'confluent-kafka-go-consumer-example/version.Version=$(VERSION)' -X 'confluent-kafka-go-consumer-example/version.BuildDate=$(BUILD_DATE)'\" -a -tags \"static netgo musl\" -installsuffix netgo -o app-amd64"
